    ...
    const handlePostComment= (event)=>{
     ...
     const handleCommentDelete = (event)=>{
        ...
        if(!error){
            if(!loading){
    return <>
        <article className="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert dark:bg-gray-900 p-16">
          <header className="mb-4 lg:mb-6 not-format">
              <address className="flex items-center mb-6 not-italic">
                  <div className="inline-flex items-center mr-3 mt-20 text-sm text-gray-900 dark:text-white">
                           <div>
                          <a href="#" rel="author" className="text-xl font-bold text-gray-900  dark:text-white">{article.author}</a>
                          <p className="text-base text-gray-500 dark:text-gray-400">{article.topic}</p>
                          <p className="text-base text-gray-500 dark:text-gray-400"><time>{article.created_at.split("T")[0]}</time></p>
                      </div>
                  </div>
              </address>
            ...
            <p className="article-body dark:text-white">{article.body}</p>
            ...
            <div className="mt-4">
                <div className="inline-flex border border-gray-200 rounded-full p-0.5 dark:border-neutral-700">
                   <button type="button" onClick={handleLike} className="inline-flex shrink-0 justify-center items-center size-8 rounded-full text-gray-500 hover:bg-blue-100 hover:text-blue-800 focus:outline-none focus:bg-blue-100 focus:text-blue-800 dark:text-neutral-500 dark:hover:bg-blue-900 dark:hover:text-blue-200 dark:focus:bg-blue-900 dark:focus:text-blue-200">
                    ...
                    <span className="like-text dark:text-white">{votes}</span> 
             ...
             <span className="ml-4 dark:text-white">{commentsCount} comments</span>
            </div>
         
            <button onClick={handleAddComment} className={`bg-gray bg-opacity-25 py-2.5 px-4  text-sm my-4 rounded-md hover:bg-yellow hover:bg-opacity-20 dark:text-white ${addCommentButton}`}>Add comment...</button>
            ...
            className="inline-flex items-center py-2.5 px-4 text-sm text-center text-gray-light bg-blue rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-green hover:text-gray-dark dark:text-white dark:bg-gray-700">
            Post comment
             </button>
            </form>    
            </div>
            ...
            <div className="dark:bg-gray-900">
                {comments.map((comment)=>{
                    return <div key={comment.comment_id} className="commentdiv">
                             ...
                             {(user===comment.author)?<button value={comment.comment_id} onClick={handleCommentDelete} className=" py-2 px-4 bg-pink rounded-md text-sm mb-4 ml-3 bg-red-800 text-gray-100">delete</button>:null}
                            </div> 
                })}
            </div>
            </article>
